...
static void MoveResizeWindowNow_(struct windowRR *lw, UINT flag)
{
    ...
    if (lw->end && !lw->maximize && (IsZoomed(hwnd) || IsWindowSnapped(hwnd))) {
        ...
        // Force immediate repaint of the window's content during resize
        // Without this, the window content doesn't update in real-time,
        // showing black/white areas instead (unlike native Windows resize)
        if (!lw->moveonly) {
